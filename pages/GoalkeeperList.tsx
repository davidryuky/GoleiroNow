...
const GoalkeeperList: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="bg-white/5 backdrop-blur-lg p-6 rounded-xl mb-10 border border-gray-700/50 grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
        <div>
        ...
        <div>
          ...
          <select id="region" name="region" value={filters.region} onChange={handleFilterChange} className="w-full bg-gray-800/50 border border-gray-700 text-white rounded-lg px-4 py-2 focus:outline-none focus:ring-0 focus:border-primary focus:shadow-[0_0_15px_rgba(126,63,242,0.5)] transition-all duration-300">
      ...
      {loading ? <Spinner /> : (
        filteredGoalkeepers.length > 0 ? (
          ...
          <p className="text-center text-gray-400 text-lg py-10">Nenhum goleiro encontrado com os filtros selecionados.</p>
        )
      )}
    </div>
  );
