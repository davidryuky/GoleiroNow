...
const GoalkeeperList: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <h1 className="text-4xl font-black tracking-tighter text-center mb-10">Encontre o Goleiro <span className="text-primary">Perfeito</span></h1>

      <div className="bg-gray-900 p-6 rounded-xl mb-10 border border-gray-700 grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
        <div>
          <label htmlFor="price" className="block text-sm font-medium text-gray-300">Preço Máximo: <span className="font-bold text-primary">R${filters.price}</span></label>
        ...
        <div>
          <label htmlFor="rating" className="block text-sm font-medium text-gray-300">Avaliação Mínima: <span className="font-bold text-primary">{filters.rating.toFixed(1)}</span></label>
        ...
        <div>
          <label htmlFor="region" className="block text-sm font-medium text-gray-300">Região</label>
          <select id="region" name="region" value={filters.region} onChange={handleFilterChange} className="w-full bg-gray-800 border border-gray-700 text-white rounded-lg px-4 py-2 focus:ring-2 focus:ring-primary focus:border-primary">
      ...
      {loading ? <Spinner /> : (
        filteredGoalkeepers.length > 0 ? (
          ...
          <p className="text-center text-gray-400 text-lg">Nenhum goleiro encontrado com os filtros selecionados.</p>
        )
      )}
    </div>
  );
